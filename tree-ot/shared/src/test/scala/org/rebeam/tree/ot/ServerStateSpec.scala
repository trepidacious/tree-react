...
class ServerStateSpec extends wordspec.AnyWordSpec with Matchers with Checkers {
  ...
  "ServerState" should {
    "handle simple case" in {
      ...
      val (s1, op0b) = s0.updated(OpRev(op0, 0, Rev(0)))
      ...
      val (s2, op1b) = s1.updated(OpRev(op1, 0, Rev(0)))
    ...
    "track edited position" in {
      ...
      val(s3, opA1s) = s2.updated(OpRev(opA1, 0, Rev(1))) // Note this is still at Rev 1, even though server is at rev 2 - this
