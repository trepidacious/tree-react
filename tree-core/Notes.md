...
3. modify: Modify the value for an id, essentially using a transformation `A => A` (actually `A=>F[A]`, more details later)
...
3. Making transactions preserve user intent. Using a `List[Ref[A]]` rather than a `List[A]` allows transactions to be "anchored" to a Ref, rather than (for example) to an index in the list. If the list is reordered on the server, before the transaction is applied, the same data will be edited. This could also be implemented by use of some "find" function to locate the correct data, using the contents of the list elements, however this may be less efficient, or may require introducing another, ad-hoc Id to find the item. Note that in this use case, OT is probably still a better solution.
...
3. Add support for OT "cursors" inside transactions. This would look something like an STMOp of form `Int => Option[Int]`. This takes an integer index in an OT list at the OT revision seen at the start of the transaction when it is first run, on the client.
   ...
   trouble of referencing elements of the list by Id. This is preferable to just using OT to delete and insert a modified item at an index, since it allows for merging of edits rather than last-edit-wins.
4. Provide a `Tag` type similar to `Id` and using the same guid system, but not allowing storage in an STM cell. This can be used to give a unique identifier to data that can be found without using the STM, but still needs to have a persistent identity. For example we might want to have a `List[Item]` where each `Item` has a `Tag` - we can find a given item by searching the list for the correct `Tag`. Alternatively we might keep a `Map[Tag[Item], Item]` for efficiency. This allows us to sidestep the STM if we don't need its full functionality.
