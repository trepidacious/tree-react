...
/**
  ...
  * with modifications to permit use of arbitrary SessionIds (they do not need to have a minimum value, just be
  ...
  *
  * The "id" is a Position, which is a list of Identifiers. An Identifier is a pos: Int, and a SessionId (which is itself
  * essentially a Long)
  *
  * ## Invariants:
  *
  ...
  *     F. Identical to firstPosition: contains a single Identifier(minPosValue, SessionId(0))
  *     L. Identical to lastPosition, contains a single Identifier(maxPosValue, SessionId(0))
  *     G. Generated by this function
  *
  *   3. Only positions of class G, which we will call generated positions, are ever explicitly present in Sequences.
  ...
  *   4. All generated positions have a final identifier with a SessionId from the client that generated them.
  *   5. All generated positions have a final identifier with a pos strictly greater than minPosValue. (Note that L
  *      also has this property, so all positions we may need to insert before have this property)
  *
  * ## Requirements:
  *
  ...
  * ## Ordering
  *
  *   1. Identifiers are a pos and a SessionId, and are compared first by pos, then by SessionId (if pos is equal)
  *      Therefore if two sessions concurrently insert a Position using a final Identifier with the same pos, we
  *      can still order the two Identifiers.
  *   2. Positions are a list of Identifiers, and are compared identifier by identifier from first to last, as long as
  *      both Positions have identifiers - as soon as an identifier is different, this gives the ordering. If all
  *      identifiers are equal up to the end of the shorter Position, and the positions are different lengths, then
  *      the longer Position is greater. If Positions have the same Identifiers and same length then they are the same.
  *
  * ## Notes
  *
  *   1. PositionIds add a Guid to a Position. The PositionId is used entirely for ensuring
  *      uniqueness and looking up PositionIds directly. They are irrelevant for Positions themselves.
  *      To see why we need a Guid, consider two cases - firstly we may want to move "an item" in a list, but
  *      allow us to still find it in the new list - for this we need to search by guid, since the Position will have
  *      changed. Secondly, consider a case where an element is deleted, and then a new element is inserter - it is
  *      possible for the original Position to be regenerated, and in this case we could not assume that the "item" in
  *      the list at that Position was the same as the previous item at the same Position.
  ...
  * use these to find them in the list for editing (or to use elements with an Id and use a List of Ref),
  ...
  * valid for locating an item within the particular Logoot list by which it is generated. The same item can exist
  * in different lists using different Guids.
  * Note that this also implies that if we have a "Cursor" in a Logoot list, we must assign an identifier to the
  * Logoot list itself, so that we can check this matches before using the Cursor.
  * Such a Cursor could work in two ways, for different use cases:
  *   1. To track a particular identifier item in a list, we would use a Sequence, and store the PositionId of the
  *      "selected" item, and the Guid associated with a Logoot List. We would then search the Sequence for the
  *      Guid of the PositionId. If the item is no longer present, we could use the Position of the PositionId to
  *      select the next item before/after the insertion index of that Position in the list, or select the first item,
  *      or no element, according to the use case / preferences.
  *   2. To track a particular position in a list, without caring what the "item" at that position is (as when editing
  *      text) we would store a Position and the Guid associated with a Logoot list. We would then find the insertion
  *      position of the Position into the list. We would place our text "caret" at this insertion position. When
  *      inserting we would use the Positions on either side as normal, and when deleting/backspacing we would delete
  *      the character to the left/right of the insertion point, referenced by PositionId.
  *
...
object Logoot {
  ...
  /**
    ...
    * SessionId value is not important for the valid operation of the Logoot. However 0
    * is conventionally used for initial data, and >= 1 for actual clients.
    */
  val lowerIdent: Identifier = Identifier(minPosValue, SessionId(0))
  ...
  /**
    ...
    * is conventionally used for initial data, and >= 1 for actual clients.
    */
  val upperIdent: Identifier = Identifier(maxPosValue, SessionId(0))
  ...
  /**
    ...
    * @param SessionId The client that generated the Identifier.
    ...
    */
  case class Identifier(pos: Int, SessionId: SessionId)
  ...
  /**
    * Compare Identifiers by pos, and then by site (using SessionId's id value)
    */
  implicit val identifierOrdering: Ordering[Identifier] = new Ordering[Identifier] {
    def compare(a: Identifier, b: Identifier): Int = {
      ...
      if (byPos == 0) {
        a.SessionId.id.compareTo(b.SessionId.id)
      } else {
        byPos
      }
    }
  ...
  object Position {
    ...
    /**
      ...
      *
      * //TODO can/should we use the SessionId parameter instead of default SessionId(0) in pIdent and qIdent?
      *
      ...
      * @param SessionId   The SessionId to be used for the final identifier in the result
      ...
      identifierOrdering.compare(pIdent, qIdent) match {
        ...
        case -1 =>
          ...
          if (posDiff > 1) {
            ...
            for {
              ri <- stm.randomIntUntil(posDiff - 1)  // From 0 (incl.) to posDiff - 1 (excl.)
            } yield {
              val random = ri + 1 // Add 1 since we need a random value from 1 (incl.) to posDiff - 1 (incl.)

              r :+ Identifier(pIdent.pos + random, SessionId)
            }
          ...
          // No gap, try based on client id differences
            ...
            val pHasLowerClient = SessionId.id > pIdent.SessionId.id
            val qHasHigherClient = SessionId.id < qIdent.SessionId.id
            ...
            if (posDiff == 0 && pInvariant5 && pHasLowerClient && qHasHigherClient) {
              pure(r :+ Identifier(pIdent.pos, SessionId))
              ...
              pure(r :+ Identifier(qIdent.pos, SessionId))
              ...
              positionBetweenRec[F](rest(p), Nil, r :+ pIdent, SessionId)
            }
          }
        ...
        case _ =>
          positionBetweenRec[F](rest(p), rest(q), r :+ pIdent, SessionId)
      }
    }
...
//      println(s">>> between $p and $q")
      for {
        context <- stm.context
        position <- positionBetweenRec[F](p, q, Nil, context.transactionId.sessionId)
      } yield {
//        println(s"...got $position")
  ...
  /**
    ...
    * we only really need its clientDeltaId and id  - we already have the SessionId
    * from the Position itself, but for consistency we use the whole thing.
    * @param position   The position
    * @param id         Id
    */
  case class PositionId(position: Position, id: Guid)
  ...
  case class Sequence[A](map: SortedMap[PositionId, A]) {
    ...
    /**
      ...
      */
    def deleted(guid: Guid): Sequence[A] =
