...
object API {
  ...
  /**
    * A View can produce a value using ViewOps, in any Monad F. This represents
    * an atomic operation p
    */
  type View[A] = Program[A, ViewOps]
  ...
  trait ViewBase extends ReadBase {
    ...
    /**
      ...
      */
    def getOption[A](id: Id[A]): View[Option[A]] = new View[Option[A]] {
      def apply[F[_]: Monad](implicit ops: ViewOps[F]): F[Option[A]] = ops.getOption(id)
    }
    ...
    /**
      ...
      */
    def getOTListCursorUpdate[A](list: OTList[A]): View[CursorUpdate[A]] = new View[CursorUpdate[A]] {
      def apply[F[_]: Monad](implicit ops: ViewOps[F]): F[CursorUpdate[A]] = ops.getOTListCursorUpdate(list)
    }
    ...
    /**
      ...
      */
    def getOTListCursorUpdateOption[A](list: OTList[A]): View[Option[CursorUpdate[A]]] = new View[Option[CursorUpdate[A]]] {
      def apply[F[_]: Monad](implicit ops: ViewOps[F]): F[Option[CursorUpdate[A]]] = ops.getOTListCursorUpdateOption(list)
    }
  }
