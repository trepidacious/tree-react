...
## Build errors from scalably-typed
...
Contains code to generate a facade for each material-ui component. This uses the same approach the material-ui project uses to generate API documentation, and so should be relatively accurate.

Code generation uses few heuristics to assign more detailed types (mostly for functions), and to add some apparently missing props (including inheritance). 
There are only a small number of tweaks needed, so hopefully this will allow for relatively easy updates to new material-ui versions.
...
Until this is published properly, you will need to clone the project, run `sbt`, then `publishLocal`. 
The generated facade is checked in so you don't need to run the code generation unless you want to work on the generator itself, or update to a new material-ui version.

To regenerate code:

1. Check out the material-ui project, in a directory named `material-ui`, alongside this project. The sources here will be used by the API extraction.
2. In the `material-ui-docgen` folder, run `yarn` then `yarn start`. This uses a .js script to run [react-docgen](https://github.com/reactjs/react-docgen) on the `material-ui` core sources, using the same logic as the `material-ui` API generation to find the correct files, and tweak the output to handle custom propTypes used by material-ui. 
3. This will create a new `muiapi.json` file in the scalajs-react-material-ui jvm resources directory.
4. Finally, run `scalajsReactMaterialUIJVM/run` from sbt.

This will generate source files for the material-ui components directly into the js src folder, under package `org.rebeam.mui`. In future this may be done as a build step, but at this stage it's useful to commit generated sources so we can see changes as the generation process is updated.
...
Icon names are taken from the .js files in the material-ui icons package src folder, pasted to a `muiicons.csv` file in jvm resources.
