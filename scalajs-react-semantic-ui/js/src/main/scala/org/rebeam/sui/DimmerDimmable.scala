...
object DimmerDimmable {
  ...
  @js.native
  object DimmerDimmableJS extends js.Object

  val jsComponent = JsComponent[Props, Children.Varargs, Null](DimmerDimmableJS)
  ...
  def apply(
    ...
    if (blurring.isDefined) {p.blurring = blurring}
    if (className.isDefined) {p.className = className}
    if (content.isDefined) {p.content = content}
    if (dimmed.isDefined) {p.dimmed = dimmed}
